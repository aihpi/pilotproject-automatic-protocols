...
# Bare footer lines the page-table regex misses once Docling drops the pipes:
# "Landtag Brandenburg" and the "P-HA 8/22" protocol number on their own line.
_LANDTAG_LINE_RE = re.compile(r"(?m)^[ \t]*(?:#{1,6}[ \t]*)?Landtag Brandenburg[ \t]*$\n?")
_DOCNUM_LINE_RE = re.compile(
    r"(?m)^[ \t]*(?:#{1,6}[ \t]*)?P-[A-ZÄÖÜ]{2,6}[ \t]+\d{1,2}/\d{1,3}[ \t]*$\n?")
# Justified-text padding from PDF extraction: 2+ spaces between non-space
# characters. Indentation at line start is left alone.
_MULTI_SPACE_RE = re.compile(r"(?<=\S) {2,}(?=\S)")
# Line-break hyphenation flattened by Docling into "Schul- schwimmen". Coordinated
# compounds ("Bildungs- und Sport") keep their hyphen and space.
_SPLIT_HYPHEN_RE = re.compile(
    r"(?<=[a-zäöüß])- +(?!(?:und|oder|bzw\.?|sowie)\b)(?=[a-zäöüß])")
...
def strip_footer_lines(text: str) -> str:
    """Drop bare 'Landtag Brandenburg' and 'P-XX n/m' footer lines."""
    return _DOCNUM_LINE_RE.sub("", _LANDTAG_LINE_RE.sub("", text))


def collapse_spaces(text: str) -> str:
    """Collapse runs of 2+ spaces inside a line to one (justified-text padding)."""
    return _MULTI_SPACE_RE.sub(" ", text)


def join_split_hyphens(text: str) -> str:
    """Rejoin words split at a former line break ('inner- halb' -> 'innerhalb')."""
    return _SPLIT_HYPHEN_RE.sub("", text)

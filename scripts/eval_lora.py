#!/usr/bin/env python3
"""Evaluation runner: summarise the data/test/ examples with one LoRA adapter.

One process loads the model + adapter once, then writes a summary for every
(example x decode-preset) into each example's ``summaries/`` folder. Pick the
backend with ``--framework``; run it under that framework's venv:

  # PEFT / FSDP adapters (stock transformers + peft)
  .venv/bin/python scripts/eval_lora.py --framework peft \
      --adapter results/20260617-200127 --adapter-id cce_bf16_nodocs_cap32k \
      --base-model google/gemma-4-31B-it --max-seq-len 32768

  # Unsloth adapters (must load through FastModel)
  ../pilotproject-altlora/.venv-unsloth/bin/python scripts/eval_lora.py \
      --framework unsloth --adapter ../pilotproject-altlora/results/20260617-182728 \
      --adapter-id unsloth_31b_qlora --max-seq-len 32768

  # Keras (.lora.h5) adapters — different/smaller base; demo of the path
  ../pilotproject-altlora/.venv-keras/bin/python scripts/eval_lora.py \
      --framework keras --adapter ../pilotproject-altlora/results/keras_20260617-132554 \
      --adapter-id keras_gemma3_1b --preset hf://google/gemma-3-1b-it --decodes baseline

Preprocessing (system+user chat template, 2-pass TOP split) is identical across
frameworks; only the adapter and the decode preset vary. This is intentionally
*more faithful to training* than the legacy infer_unsloth.py (which crammed the
system prompt into the user turn and used a cruder split), so the Unsloth output
here may differ from tmp/HANDOFF_repetition_fix.md.
"""
...
# --- Keras / JAX backend ------------------------------------------------------
def backend_keras(args):
    import keras_hub  # noqa: F401

    preset = args.preset or _read_base_from_adapter(args.adapter) or "hf://google/gemma-3-1b-it"
    print(f"[keras] from_preset {preset}", file=sys.stderr, flush=True)
    # gemma-3 uses Gemma3Backbone -> Gemma3CausalLM, not GemmaCausalLM. Use the
    # generic CausalLM dispatcher which resolves the right subclass per preset.
    model = keras_hub.models.CausalLM.from_preset(preset)
    model.backbone.enable_lora(rank=args.lora_rank)
    weights = args.adapter / "model.lora.h5"
    print(f"[keras] loading LoRA weights {weights}", file=sys.stderr, flush=True)
    # train_lora_keras.py saved via backbone.save_lora_weights(); mirror it.
    model.backbone.load_lora_weights(str(weights))
    warned = {"rep": False}

    def generate_fn(system: str, user: str, k: dict) -> str:
        if k["repetition_penalty"] != 1.0 and not warned["rep"]:
            print("  [keras] repetition_penalty not supported by KerasHub sampler; ignored",
                  file=sys.stderr)
            warned["rep"] = True
        prompt = (system + "\n\n" + user).strip()
        full = model.generate(prompt, max_length=args.max_seq_len)
        return full[len(prompt):].strip() if full.startswith(prompt) else full.strip()

    return generate_fn, str(preset)


BACKENDS = {"peft": backend_peft, "unsloth": backend_unsloth, "keras": backend_keras}
...
def main() -> int:
    ...
    p.add_argument("--preset", default=None, help="KerasHub preset (keras backend)")
    p.add_argument("--lora-rank", type=int, default=8, help="Keras LoRA rank (default: 8)")
